...
const PerformanceView = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/performances')
      .then(res => res.json())
      .then(data => {
        setAllData(data);
        setDisplayData(data);
      ...
      <div className="ai-enhanced-view">
        <div className="ai-loading-overlay" style={{ position: 'relative', background: 'transparent', backdropFilter: 'none' }}>
          <div className="ai-loading-content" style={{ boxShadow: 'none' }}>
            ...
            <p>Veritabanından performans verileri yükleniyor...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="ai-search-card">
        ...
        <div className="ai-search-input-group">
          <input
            ref={inputRef}
            type="text"
            placeholder="Örn: A Blok'taki tamamlanan asansör bakım sürelerini listele..."
            value={queryText}
          ...
          <table className="ai-table">
            ...
            <tbody>
              {displayData.length === 0 ? (
                <tr><td colSpan="8" className="ai-table-empty">Bu filtreye uygun kayıt bulunamadı.</td></tr>
              ) : (
                displayData.map((perf, index) => (
                  <tr key={perf._id}>
                    <td>{index + 1}</td>
                    <td className="ai-td-subject">{perf.subject}</td>
                    <td><span className="ai-category-tag">{CATEGORY_LABELS[perf.category] || perf.category}</span></td>
                    <td>{perf.building}</td>
                    <td>{perf.zone}</td>
                    <td>{perf.duration}</td>
                    <td>{perf.createdAt ? new Date(perf.createdAt).toLocaleDateString('tr-TR') : '-'}</td>
                    <td>{getBadge(getDynamicStatus(perf))}</td>
                  </tr>
                ))
              )}
            </tbody>
